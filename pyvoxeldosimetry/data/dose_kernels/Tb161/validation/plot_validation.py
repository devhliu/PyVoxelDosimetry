...
def plot_validation_comparisons():
    """Plot validation comparisons for Tb161 kernel."""
    validation_dir = Path(__file__).parent
    
    # Load validation data
    mird_data = pd.read_csv(validation_dir / 'Tb161_mird_comparison.csv')
    gate_data = pd.read_csv(validation_dir / 'Tb161_gate_comparison.csv')
    fluka_data = pd.read_csv(validation_dir / 'Tb161_fluka_comparison.csv')
    ...
    ax1.set_title('Tb-161 Dose Kernel Validation')
    ax1.legend()
    ...
    plt.tight_layout()
    plt.savefig(validation_dir / 'Tb161_validation_comparison.png', dpi=300)
    plt.close()
